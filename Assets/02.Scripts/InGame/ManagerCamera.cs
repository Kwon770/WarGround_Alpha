...
public class ManagerCamera : MonoBehaviour {
    ...
    private void Awake()
    {
        ...
        if (!PhotonNetwork.isMasterClient)
        {
            ...
            {
                Debug.Log(grass);
                grass.transform.Rotate(0, 180, 0);
            }
        }
    }
    ...
    bool CameraBack()
    {
        if (Vector3.Magnitude(Center.position - transform.position) > distance)
        {
            ...
            transform.Translate((Center.position - transform.position) * Time.deltaTime * CamerBackSpeed * Vector3.Magnitude(Center.position - transform.position) * isRotate);
            return true;
        }
        return false;
    }
